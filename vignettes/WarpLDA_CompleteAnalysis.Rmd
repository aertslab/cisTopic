...
vignette: >
  %\VignetteIndexEntry{cisTopic: Probabilistic modelling of single cell epigenomes , message=FALSE}
...
```{r , message=FALSE}
cisTopicObject <- readRDS('cisTopicObject_melanoma_warpLDA.Rds')
```
...
In this tutorial, we will test models with 2 to 15 (by 1), and 20 to 50 (by 5) topics [Reference running time for the example data set (100 cells; ~110,000 regions): 54 seconds].
...
- The perplexity of each model (only for WarpLDA models) measures
...
```{r, fig.show='hold', fig.align='center' , message=FALSE}
ontologyDotPlot(cisTopicObject, top=5, topics=c(1,2), var.y='name', order.by='Binom_Adjp_BH')
```
...
```{r, eval=FALSE, , message=FALSE , message=FALSE}
...
```{r, eval = FALSE , message=FALSE}
date()
pathToFeather <- "/feather/hg19-regions-9species.all_regions.mc9nr.feather"
...
```{r, eval=FALSE , message=FALSE}
cisTopicObject <- getCistromeEnrichment(cisTopicObject, topic=2, TFname='FOSL1', aucellRankings = aucellRankings, aucMaxRank = 0.05*nrow(aucellRankings), plot=FALSE)
cisTopicObject <- getCistromeEnrichment(cisTopicObject, topic=3, TFname='FOSL1', aucellRankings = aucellRankings, aucMaxRank = 0.05*nrow(aucellRankings), plot=FALSE)
```

```{r, fig.show='hold', fig.align='center' , message=FALSE}
par(mfrow=c(1,2))
plotFeatures(cisTopicObject, method='Umap', target='cell', topic_contr=NULL, colorBy=c('Topic2_FOSL1 (49p)','Topic3_FOSL1 (190p)'), cex.legend = 0.8, factor.max=.75, dim=2, legend=TRUE, intervals=20)
```
...
```{r, eval = FALSE , message=FALSE}
saveRDS(cisTopicObject, file='cisTopicObject_melanoma_warpLDA.Rds')
```
